...
public class NicoNicoEBooksChapter extends Chapter
{
    ...
    private transient String userid, bookid, dl_key, maki_address;
    ...
    public NicoNicoEBooksChapter(URL _url, String _username, char[] _password)
    {
        ...
        userid = bookid = dl_key = maki_address = null;
    ...
    public ArrayList<DownloadJob> init() throws Exception
    {
        ...
        login = new LoginDownloadJob("Login to NicoNico", LoginDownloadJob.Type.NICONICO, username, password)
        {
            protected void addAllPOSTData()
            {
                
            }
        };

        NicoJSONDownloadJob service = new NicoJSONDownloadJob("Get service info",
                                            new URL("http://bkapi.seiga.nicovideo.jp/service/status"));

        NicoJSONDownloadJob user = new NicoJSONDownloadJob("Get user info",
        ...
        NicoJSONDownloadJob book = new NicoJSONDownloadJob("Get book info",
        ...
        NicoJSONDownloadJob lastRead = new NicoJSONDownloadJob("Get last read",
            ...
            public void doZipEntryInput(ZipInputStream input, ZipEntry e) throws Exception
            {
                ...
                for(Element elm : d.select("img"))
                    if(elm.hasAttr("class") && elm.attr("class").equals("img-screen"))
                        images.add(URLDecoder.decode(elm.attr("src"), "UTF-8"));
            }
        };
    ...
    public ArrayList<DownloadJob> download(File directory) throws Exception
    {
        ...
        final LoginDownloadJob renewLogin = new LoginDownloadJob("Renew login", LoginDownloadJob.Type.NICONICO, username, password)
        {
            @Override
            public void run() throws Exception
            {
        ...
        {
            final File f = DownloaderUtils.fileName(directory, i + 1,
    ...
    private class NicoJSONDownloadJob extends JSoupDownloadJob
    {
        protected JSONObject obj;

        public NicoJSONDownloadJob(String _desc, URL _url)
        {
            super(_desc, _url);
        }
    ...
    private class BookInfoDownloadJob extends NicoJSONDownloadJob
    {
